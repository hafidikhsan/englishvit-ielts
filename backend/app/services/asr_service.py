...
class EvASRService:
    ...
    # Load the model thread-safely and only once
    def get_model(self):
        if self.model is None:
            with EvASRService._instance_lock:
                if self.model is None:
                    ev_logger.info(f"Loading Whisper model '{self.model_name}' ...")
                    self.model = whisper.load_model(self.model_name)
                    ev_logger.info(f"Whisper model '{self.model_name}' loaded √")
        return self.model
    ...
    def transcribe(self, audio_file_path: str):
        try:
            ...
            model = self.get_model()  # Corrected method call to _get_model()
